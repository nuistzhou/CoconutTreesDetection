...
def extract_bovw_features(imgArrayList, codebook):
    """Final BoVW feature descriptors extractor!"""
    bovwDescriptorList, labels = bovw_utils.extract_local_descriptor_ImageArrayList(imgArrayList)
    bovwDataset = get_bovw_descriptors(bovwDescriptorList, codebook)
    labelsList = np.array(labels).astype(np.uint32)
    return bovwDataset, labelsList
